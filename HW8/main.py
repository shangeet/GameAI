...
class ReplayMemory(object):
    ...
    def push(self, state, action, next_state, reward):
        ...
        info = Transition(state, action, next_state, reward)
        
        idx = self.position % self.capacity
        if len(self.memory) < self.capacity:
            self.memory.append(info)
        else:
            self.memory[idx] = info
        self.position += 1
...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        kernel  = 5
        padd = 2
        self.convOne = torch.nn.Conv2d(3, 20, kernel_size=kernel, stride=strideLen)
        self.batchNormOne = torch.nn.BatchNorm2d(3)
        self.convTwo = torch.nn.Conv2d(20, 32, kernel_size=kernel, stride=strideLen)
        self.batchNormTwo = torch.nn.BatchNorm2d(20)

        outW1 = ((self.width - (kernel - 1) - 1) // strideLen) + 1
        outH1 = ((self.height - (kernel - 1) - 1) // strideLen) + 1

        outW2 = ((outW1 - (kernel - 1) - 1) // strideLen) + 1
        outH2 = ((outH1 - (kernel - 1) - 1) // strideLen) + 1
        self.outW2 = outW2
        self.outH2 = outH2

        self.fulConOne = torch.nn.Linear(32 * outW2 * outH2, 32)
        self.fulConTwo = torch.nn.Linear(32, num_actions)
    ...
    def forward(self, x):
        ...
        #print("A", q_values.size())
        q_values = self.batchNormOne(x)
        q_values = F.relu(self.convOne(q_values))
        #print("B", q_values.size())
        #q_values = q_values.view(-1, int(self.outputSize * (self.width/2) * (self.height/2)))
        #print("C", q_values.size())
        q_values = self.batchNormTwo(q_values)
        q_values = F.relu(self.convTwo(q_values))
        #print("D", q_values.size())
        #print("E", q_values.size())
        q_values = q_values.view(-1, 32 * self.outW2 * self.outW2)
        #print("F", q_values.size())
        q_values = F.relu(self.fulConOne(q_values))
        #print("G", q_values.size())
        q_values = self.fulConTwo(q_values)
...
def select_action(state, policy_net, num_actions, epsilon, steps_done = 0, bootstrap_threshold = 0):
    ...
    if steps_done > bootstrap_threshold:
        new_epsilon = max(0.01, new_epsilon * 0.99999)
    #print("NUM ACTIONS: ", num_actions)
    if np.random.uniform() > epsilon:
        q_values = policy_net.forward(state)
        new_action = torch.argmax(q_values, dim=1)[0]
        #print("NEW ACTION A: ", new_action)
    else:
        new_action = int(round(np.random.randint(0, num_actions)))
        #print("NEW ACTION B: ", new_action)
...
def doMakeBatch(replay_memory, batch_size):
    ...
    non_final_mask = None
    ### WRITE YOUR CODE BELOW HERE
    memory_batch = replay_memory.sample(batch_size)
    batch = Transition(*zip(*memory_batch))
    states_batch =  torch.autograd.Variable(torch.cat(batch.state))
    actions_batch = torch.autograd.Variable(torch.cat(batch.action))
    rewards_batch = torch.autograd.Variable(torch.cat(batch.reward))
    non_final_mask = torch.BoolTensor([(b is not None) for b in batch.next_state])
    next_states_batch = non_final_next_states = torch.autograd.Variable(torch.cat([s for s in batch.next_state if s is not None]))

    #print("State batch size: ", states_batch.size())
    #print("Action batch size: ", actions_batch.size())
    #print("Next State batch size: ", next_states_batch.size())
    #print("Rewards batch size: ", rewards_batch.size())
    #print("Mask batch size: ", non_final_mask.size())
    #print(non_final_mask.sum(), batch_size//2)
...
def doPredictQValues(policy_net, states_batch, actions_batch):
    ...
    #print("doPredictQVals: ", state_action_values.size())
    ### WRITE YOUR CODE ABOVE HERE
...
def doPredictNextStateUtilities(policy_net, next_states_batch, non_final_mask, batch_size):
    ...
    ### WRITE YOUR CODE BELOW HERE
    q_values = policy_net(next_states_batch)
    next_state_values = torch.autograd.Variable(torch.zeros(batch_size).type(torch.Tensor))
    next_state_values[non_final_mask] = q_values.max(1)[0]
    #next_state_values = next_state_values.int()
    #print("do predict next state utilities: ", next_state_values.size())
    #print(next_state_values.int())
    ### WRITE YOUR CODE ABOVE HERE
...
def doComputeLoss(state_action_values, expected_state_action_values):
    ...
    loss = func(state_action_values.unsqueeze(1), expected_state_action_values.unsqueeze(1))
    ### WRITE YOUR CODE ABOVE HERE
    return loss
...
def doBackprop(loss, parameters):
    ...
    #parameters = torch.from_numpy(list(parameters))
    #print(parameters.grad)
    #ans = list(parameters)[0].data.numpy()
...
def train(num_episodes = NUM_EPISODES, load_filename = None, save_filename = "monsterModel", eval_interval = EVAL_INTERVAL, replay_capacity = REPLAY_CAPACITY, bootstrap_threshold = BOOTSTRAP, epsilon = EPSILON, eval_epsilon = EVAL_EPSILON, gamma = GAMMA, batch_size = BATCH_SIZE, target_update = TARGET_UPDATE, random_seed = RANDOM_SEED, num_levels = NUM_LEVELS, seed = SEED):
    ...
    while i_episode < num_episodes:
        ...
        if steps_done > bootstrap_threshold and i_episode > 0 and i_episode % eval_interval == 0:
            ...
            print("Best Eval so far: ", best_eval)
